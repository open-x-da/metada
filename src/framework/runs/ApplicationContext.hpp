 ...
 * - MPI Runtime: MPI initialization and finalization (when METADA_USE_MPI is
 * enabled)
 ...
 * - Threading Configuration: Threading configuration (planned)
 *
 * @par Key Features
 ...
 * @note Services are initialized in order: MPI -> Config -> Logger -> Future
 * Services
...
class ApplicationContext {
 public:
   ...
   *
   * @param argc Number of command line arguments (may be modified by MPI_Init)
   * @param argv Array of command line arguments (may be modified by MPI_Init)
   ...
   * 1. MPI initialization (if METADA_USE_MPI is enabled)
   * 2. Check if config file path is provided in command line
   * 3. Configuration loading from specified file
   * 4. Logger initialization with configuration
   * 5. Logs successful initialization with application name
   */
  ApplicationContext(int& argc, char**& argv)
      : config_(validateAndGetConfigPath(argc, argv)) {
    // Initialize MPI first (before any other services that might need it)
   ...
   * 2. Logger
   * 3. Configuration
   * 4. MPI finalization (if METADA_USE_MPI is enabled)
   */
  ~ApplicationContext() {
    Logger<BackendTag>::Instance().Info() << "Application context destroyed";
    // Finalize MPI last (after all other services are cleaned up)
 ...
 private:
   ...
   */
  static const char* validateAndGetConfigPath(int argc, char** argv) {
    if (argc < 2) {

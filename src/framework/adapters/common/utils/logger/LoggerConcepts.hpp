 ...
 */
template <typename T>
concept HasLogMessage = requires(T t, LogLevel level, const std::string& message) {
 ...
 * 
 * A logger backend must have a LogMessage method that takes a LogLevel and string message.
 * It may optionally have static Init and Shutdown methods for lifecycle management.
 */
template <typename T>
concept LoggerBackend = HasLogMessage<T> && 

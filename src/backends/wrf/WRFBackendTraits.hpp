#pragma once
#include "../../traits/BackendTraits.hpp"

// Include backend headers based on CMake configuration
#ifdef CONFIG_BACKEND_JSON
#include "../common/utils/config/json/JsonConfig.hpp"
#endif

#ifdef CONFIG_BACKEND_YAML
#include "../common/utils/config/yaml/YamlConfig.hpp"
#endif

#ifdef LOGGER_BACKEND_CONSOLE
#include "../common/utils/logger/console/ConsoleLogger.hpp"
#endif

#ifdef LOGGER_BACKEND_NGLOG
#include "../common/utils/logger/nglog/NgLogger.hpp"
#endif

// BufrObsIO is always available (includes stub when BUFR is disabled)
#include "../common/io/BufrObsIO.hpp"
#include "../common/observation/GridObservation.hpp"
#include "../common/observation/PrepBUFRObservation.hpp"
#include "../common/observation/PrepBUFRObservationIterator.hpp"
#include "WRFBackgroundErrorCovariance.hpp"
...
#include "WRFObsOperator.hpp"
#include "WRFObservation.hpp"
 ...
 * implementation types for each backend component. These implementations
 * provide integration with the Weather Research and Forecasting (WRF) model
 * system, including its geometry, state vector, and model components.
 */
template <>
...
/** @brief Logger backend implementation */
  ...
  using IncrementBackend =
      backends::wrf::WRFIncrement<ConfigBackend, GeometryBackend>;
  ...
  using ObservationIteratorBackend =
      metada::backends::common::observation::PrepBUFRObservationIterator;

  /** @brief WRF observation operator backend implementation */
  using ObsOperatorBackend =
      backends::wrf::WRFObsOperator<StateBackend, ObservationBackend,
                                    IncrementBackend, ControlVariableBackend>;

  /** @brief WRF-specific model backend implementation */
  using ModelBackend =
      backends::wrf::WRFModel<ConfigBackend, StateBackend, IncrementBackend>;

  /** @brief WRF-specific background error covariance backend implementation */
  using BackgroundErrorCovarianceBackend =
      backends::wrf::WRFBackgroundErrorCovariance<ConfigBackend>;
};

...
protected:
    void SetUp() override {
        google::InitGoogleLogging("cuda_test");
    }
...
TEST_F(CUDATest, DeviceProperties) {
    ...
    for (int i = 0; i < deviceCount; ++i) {
        cudaDeviceProp prop;
        checkCudaError(cudaGetDeviceProperties(&prop, i), "Failed to get device properties");

        LOG(INFO) << "Device " << i << ": " << prop.name;
        LOG(INFO) << "  Compute capability: " << prop.major << "." << prop.minor;
        LOG(INFO) << "  Total global memory: " << prop.totalGlobalMem / (1024*1024) << " MB";
        LOG(INFO) << "  Max threads per block: " << prop.maxThreadsPerBlock;
        
        // Verify minimum requirements
        EXPECT_GE(prop.major, 3) << "Device compute capability too low";
        EXPECT_GT(prop.totalGlobalMem, 1024*1024*1024ULL) << "Insufficient GPU memory";
    }
}

...
bool find_log_file(const std::filesystem::path& dir,
...
std::string read_log_file(const std::filesystem::path& dir,
                          const std::string& severity) {
  for (const auto& entry : std::filesystem::directory_iterator(dir)) {
    if (entry.path().string().find(severity) != std::string::npos) {
...
TEST_F(GLogTest, CheckMacros) {
  ...
  EXPECT_DEATH(
      { CHECK_EQ(x, 0) << "This should cause fatal error"; }, "Check failed");
}
